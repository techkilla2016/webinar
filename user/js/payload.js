...
var channel = "demo";
var appId = "9f492eda476a43ceb0324c72d3e8ce57";
var key = "0069f492eda476a43ceb0324c72d3e8ce57IAA0vdv8kPGZV7iKXkYwxHSxeeuJpDvLMBbIMMUx9bVWHKDfQtYAAAAAEABfqfzl3SfJXgEAAQDZJ8le";
...
//	return;
  ...
  if($("#username").val() == "techkilla" && $("#pass").val() == "123")
  {
	  admin = true;
	  joinChannel("99999");
	  removeLoginForm({"uid":"99999","name":"admin"});
	  checkOnlineUsers();
  }
   else
   {	   
...
var interval2 = setInterval(checkOnlineUsers,3000);
...
{       
        ...
        myid = details["uid"];
		var params = {"appID":appId,"channel":channel,"token":key,"codec":"vp8","mode":"live","uid":Number(myid)};
        rtc.params = params;       
	    join(rtc, params);
...
function createCard(user,animate=false)
{
	//trace(getIndex(user.userId.trim()));
	 ...
	 card.find("#local_stream_" + user.userId.trim()).css("opacity","0");
	
	  $.ajax({
      url: host + '/demo/helper/getUserDetails.php?uid=' + user.userId,
      success: function (response) {
      var details = JSON.parse(response);		  
	  card.find("#userImg").attr("src",host + "/demo/registration/" + details.img);
	  card.find("#frameImg").attr("src","assets/Conference Social Media-0" + (count % 5).toString() + ".png");
...
function createCard2(user)
{
	if(getIndex(user.uid) > -1) return;
	 ...
	 card.find("#local_stream_" + user.uid).css("opacity","0");
     card.css("opacity","1");
...
$(this).keypress(function(e)
{
	...
	if(e.keyCode == 32)
	{
		...
		messageChannel.sendUserMessage("0", null, null, function(message, error) {
            if (error) {
                return;
			}
       })
	   ...
	   for(var i=0;i<ousers.length;i++)
	   {
		   $("#card_" + ousers[i]).addClass("blurBg");
	   }
	}
});
...
channelHandler.onMessageReceived = function(channel, message) {
	...
	}else if(message.message == "card_" + myid + "_1")
	{
		...
		$("#local_stream_" + rtc.params.uid).css("opacity","1");
	}else if("0")
	{
		trace("halt message recevied");
		halt();
	}
...
function halt()
{
	if(streamApproved == true)
	{
	   ...
	   $("#local_stream_" + myid).css("opacity","0");
	   unpublish(rtc);
	   streamApproved = false;
	}
    ...
    function handleEvents (rtc) {
      ...
      rtc.client.on("stream-subscribed", function (evt) {
		...
		if(id != myid)
		{
		   ...
		   $("#local_stream_" + id).css("opacity","1");
		}
